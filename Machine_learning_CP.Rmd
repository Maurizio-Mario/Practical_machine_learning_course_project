...
train_cut <- train[, ((na_count)/nrow(train)) < 0.9]
dim(train_cut)

names_tr <- names(train_cut)
names_su <- names(submit_test)
...
levels(submit_test$classe) <- levels(training$classe)

dim(submit_test); dim(train)
```
